...
otvet_all = {'drink4':'beer', 'anim4':'dog', 'cigar5':'pallmall', 'house1':'yellow', 'nac2':'english', 'drink1':'water', 'cigar2':'philipmorris', 'nac5':'Schwed', 'house4':'white', 'anim3':'bird', 'drink3':'milk', 'anim1':'fish', 'cigar4':'marlboro', 'anim5':'cat', 'nac3':'Norway', 'cigar1':'rothmans', 'house5':'green', 'house2':'blue', 'drink2':'tea', 'drink5':'cafe', 'nac1':'german', 'nac4':'denmark', 'house3':'red', 'cigar3':'dunhill', 'anim2':'horse'}
...
def prov_otvet():
	for key in form:
		if otvet_all[key] != form[key].value:
	#		print(otvet_all[key], form[key].value)
			return '<p>Группа захвата выехала, но у подозреваемого было алиби и Снегурочки там не оказалось. </p><p> <font color=#ff0000> Чере 5 минут попробуйте еще раз.</font></p>'
	con = sqlite3.connect('qvest2.db')
	cur = con.cursor()
	base = (date, name)
	sql = 'UPDATE comandTable set timeFinish = ? where id = ?'
	try:
		...
		cur.close
		con.close
	return 'Группа захвата выехала и нашла у преступников Снегурочку Ура.'
	#print("'{}':'{}', ".format(key,form[key].value))

if form: print(prov_otvet())


for row in ulika:
	if sum_ulika(row[1]) == sum_lut(row[1]): 
		xc += '<li>{}</li>'.format(text_ulika[row[1]-1])
		ul +=1
...
if ul != 14:
	...
	print('Ваша команда закончила игру ', user[2])
else:
	print('''
